...
def main():
    ...
    if selected_model == "Primary Care":
        ...
        with st.container(border=True):
            ...
            aht_seconds = st.number_input(label="Enter AHT seconds between 0 and 59", min_value=0, max_value=59, step=1, value=40)
        with st.container(border=True):
            st.text("3. Not Ready Rate")
            not_ready = st.number_input(label="Enter Not Ready Rate between 15 and 35", min_value=15.0, max_value=35.0, step=0.1, value=21.3)
        with st.container(border=True):
            st.text("4. FTEs Logged In (Use Power BI CNC Call Metrics Staffing as a guide)")
            ftes_logged_in = st.number_input(label="Enter FTEs between 20 and 45", min_value=20.0, max_value=45.0, step=0.1, value=24.0)
        ...
        st.sidebar.caption("Data timeframes: 10/3/2022-7/2/2024")
        st.sidebar.caption("Current accuracy: 90.2%")
        ...
        with st.container(border=True):
            ...
            aht_seconds = st.number_input(label="Enter AHT seconds between 0 and 59", min_value=0, max_value=59, step=1, value=4)
        with st.container(border=True):
            st.text("3. Not Ready Rate")
            not_ready = st.number_input(label="Enter Not Ready Rate between 15 and 35", min_value=15.0, max_value=35.0, step=0.1, value=25.0)
        with st.container(border=True):
            st.text("4. FTEs Logged In (Use Power BI CNC Call Metrics Staffing as a guide)")
            ftes_logged_in = st.number_input(label="Enter FTEs between 7 and 17", min_value=7.0, max_value=17.0, step=0.1, value=14.2)
        not_ready_con = not_ready/100
        aht = aht_minutes + (aht_seconds/60)
        ...
        with st.container(border=True):
            ...
            if sl_prediction <= 0:
                ...
                st.subheader(f"{sl_prediction}%")
        st.sidebar.caption("Model: eXtreme Gradient Boosting (XGBoost)") 
        st.sidebar.caption("Data timeframes: 6/3/2022-7/2/2024")
        st.sidebar.caption("Current accuracy: 85.4%")


    elif selected_model == "Heart Care":
        with st.container(border=True):
            st.text("1. Call Volumes")
            calls_offered = st.number_input(label="Enter a call volume between 300 and 1400", min_value=300, max_value=1400, step=1, value=798)
        with st.container(border=True):
            st.text("2. Average Handle Time")
            aht_minutes = st.number_input(label="Enter AHT minutes between 4 and 7", min_value=4, max_value=7, step=1, value=5)
            aht_seconds = st.number_input(label="Enter AHT seconds between 0 and 59", min_value=0, max_value=59, step=1, value=25)
        with st.container(border=True):
            st.text("3. Not Ready Rate")
            not_ready = st.number_input(label="Enter Not Ready Rate between 15 and 35", min_value=15.0, max_value=35.0, step=0.1, value=21.3)
        with st.container(border=True):
            st.text("4. FTEs Logged In (Use Power BI CNC Call Metrics Staffing as a guide)")
            ftes_logged_in = st.number_input(label="Enter FTEs between 7 and 17", min_value=7.0, max_value=18.0, step=0.1, value=13.9)
        not_ready_con = not_ready/100
        aht = aht_minutes + (aht_seconds/60)
        ...
        with st.container(border=True):
            ...
            if sl_prediction <= 0:
                ...
                st.subheader(f"{sl_prediction}%")
        st.sidebar.caption("Model: eXtreme Gradient Boosting (XGBoost)") 
        st.sidebar.caption("Data timeframes: 1/3/2022-7/2/2024")
        st.sidebar.caption("Current accuracy: 93.0%")


    elif selected_model == "MA CRT Team":
        with st.container(border=True):
            st.text("1. Call Volumes")
            calls_offered = st.number_input(label="Enter a call volume between 200 and 1100", min_value=200, max_value=1100, step=1, value=496)
        with st.container(border=True):
            st.text("2. Average Handle Time")
        ...
        with st.container(border=True):
            st.text("3. Not Ready Rate")
            not_ready = st.number_input(label="Enter Not Ready Rate between 20 and 45", min_value=20.0, max_value=45.0, step=0.1, value=34.3)
        with st.container(border=True):
            st.text("4. FTEs Logged In (Use Power BI CNC Call Metrics Staffing as a guide)")
            ftes_logged_in = st.number_input(label="Enter FTEs between 4 and 15", min_value=4.0, max_value=15.0, step=0.1, value=8.2)
        not_ready_con = not_ready/100
        aht = aht_minutes + (aht_seconds/60)
